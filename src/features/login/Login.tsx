      ...
      <div className="container flex rounded-lg overflow-hidden shadow-md">
        <section className="w-8/12 p-5 flex flex-col bg-apple-gray-6">
          ...
          <p className="text-sm text-end">
            Copyright © 2023 MWG Inc. All rights reserved.
          </p>
        </section>
        <LoginSection />
      </div>
...
function LoginSection() {
  ...
  async function handleLogin() {
    // const account = useState<Account>({username: inputs.username.value , password: inputs.password.value});
    const account: Account = {
      username: inputs.username.value,
      password: inputs.password.value,
      token: "",
    };
    const api: Account = await APIAccount.login(account);
    console.log(api);
